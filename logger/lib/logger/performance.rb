...
#   { title: method name, message: first arg, details: remaining args }.
...
class Logger
  class Performance
    ...
    SPECIAL_METHODS = [:search, :view, :event, :rule, :execute].freeze
...
#         }
      ...
      def load_config args
        ...
        args[:methods] = default_methods_config if args[:methods] == :default
      ...
      def with_timer method, args
        update_context args[:context] if args[:context]
      ...
      def update_context new_context
        ...
        if @@context_entries.empty? ||
          ...
          @@context_entries << new_entry(
            title: "process", message: new_context, context: new_context
          )
        end
      end
      ...
      def finish_context
        ...
        # we don't know whether the next entry will belong to the same context
        # or will start a new one so we save the time
      ...
      def prepare_methods_for_logging args
        ...
        classes.each do |klass, method_types|
          ...
          method_types = hashify_and_verify_keys(
            method_types, DEFAULT_METHOD_TYPE
          ) do |key|
